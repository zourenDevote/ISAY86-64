...
public:
    ...
    /// 获取上一个pc的值
    Quad getOldPcValue() const { return oldPc; }
    /// 获取标志位的状态
    Flag getFlags() const { return flags; }
    /// 解析和执行指令
    ErrorOrSuccess parseAndExecute(const MachineInst& ins);
public:
    ///打印内存分布
    ///@param start 起始地址
    ///@param end   结束地址
...
private:
    ...
    /// 保存指令仿真器原有的数据信息
    void saveOldRegsAndFlags(){ oldGroup = regGroup; oldFlags = flags; oldPc = pc;}
